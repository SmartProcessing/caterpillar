...
handle_call({storage_list_packages, Ident}, From, State) ->
    erlang:spawn(
        fun() -> 
            gen_server:reply(From, {ok, dets:match(State#state.storage, {{Ident, '$1'}, '$2', ['$3'|'_']})})
        end),
    {noreply, State};
        ...
        fun() -> 
            gen_server:reply(From, {ok, 
                    dets:select(State#state.storage, 
                        [{{Ident, '$1', {'$2', '$3'}}, '$4', '$5', '$6', '_', '_', '_'}, [{'>', '$1', State#state.work_id-?LIST_LENGTH}], []])})
        end),
    {noreply, State};
...
handle_cast({store_start_build, [Ident,
            {Name, Branch},
            WorkId,
            CommitHash
...
handle_cast({store_progress_build, [Ident,
            {Name, Branch},
            WorkId,
            Description
        ]}, State=#state{storage=S}) ->
...
handle_cast({store_error_build, [Ident,
            {Name, Branch},
            WorkId,
            BuildLog
        ]}, State=#state{storage=S}) ->
